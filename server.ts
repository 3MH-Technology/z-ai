...
async function notifyTelegramWithFiles(
  ...
  try {
    ...
    const summary = `
🚀 <b>ملفات مرفوعة جديدة - White Wolf</b>
...
async function startServer() {
  ...
  app.post('/api/send-verification', async (req, res) => {
    ...
    try {
      ...
      const mailOptions = {
        from: `"White Wolf" <${emailUser}>`,
        to: email,
        subject: 'رمز التحقق الخاص بك - White Wolf',
        html: `
          <div style="font-family: sans-serif; direction: rtl; text-align: right; padding: 20px; background-color: #f9f9f9; border-radius: 15px;">
            <div style="background-color: #2563eb; padding: 20px; border-radius: 15px 15px 0 0; text-align: center;">
              <h1 style="color: #ffffff; margin: 0; font-size: 24px;">White Wolf</h1>
            </div>
            <div style="padding: 30px; background-color: #ffffff; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 15px 15px;">
              ...
              <p style="color: #4b5563; font-size: 16px; line-height: 1.6;">شكراً لتسجيلك في منصة White Wolf. يرجى استخدام الرمز التالي لإتمام عملية التحقق من بريدك الإلكتروني:</p>
            ...
            <div style="text-align: center; padding: 20px; color: #9ca3af; font-size: 12px;">
              &copy; 2026 White Wolf t.me/j49_c channel t.me/bshshshkk
            </div>
          </div>
        `,
      };
  ...
  app.post('/api/execute', async (req, res) => {
    ...
    (async () => {
      const tempDir = path.join(os.tmpdir(), `whitewolf-exec-${serverId}-${Date.now()}`);
